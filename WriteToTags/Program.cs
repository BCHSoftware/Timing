...
namespace WriteToTags
{
    ...
    static void Main(string[] args)
    {
        try
        {
            ...
            for (ushort i = 1; i <= 4; i++) // Assuming up to 4 antennas
            {
                ...
                settings.Antennas.GetAntenna(i).TxPowerInDbm = 30.0; // Max power (adjust as needed)
    ...
    static void OnTagsReported(ImpinjReader sender, TagReport report)
    {
        foreach (Tag tag in report.Tags)
        {
            ...
            lock (lockObject)
            {
                if (programmedEpcs.Contains("tag.Epc.To "))
                    continue;
            }
            ...
            lock (lockObject)
            {
                ...
                newEpcHexString = $"30000000000000000000{tagCounter:X12}";
    ...
    static void ProgramTagEpc(string currentEpcHexString, ushort currentPcBits, string newEpcHexString)
    {
        ...
        seq.TargetTag.Data = TagData.FromHexString(currentEpcHexString);
        ...
        // Add the original EPC to the list of programmed EPCs to avoid re-programming it repeatedly
        lock (lockObject)
        {
    ...
    static void OnTagOpComplete(ImpinjReader reader, TagOpReport report)
    {
        foreach (TagOpResult result in report.Results)
        {
            ...
            {
                if (writeResult.id == 123) // Our EPC write operation
                {
                    Console.WriteLine($"  -> EPC write for {writeResult.Epc.ToHexString()} complete. Status: {writeResult.Result}");
                    if (writeResult.Result == TagOperationResultType.Success)
                    {
                        Console.WriteLine($"  -> Tag {writeResult.Epc.ToHexString()} successfully programmed!");
                    }
                    else
                    {
                        Console.WriteLine($"  -> Failed to program tag {writeResult.Epc.ToHexString()}. Error: {writeResult.StatusMessage}");
                        // If a write fails, remove from programmedEpcs so it can be reattempted
                        lock (lockObject)
                        {
                            programmedEpcs.Remove(writeResult.Epc.ToHexString());
                        }
                    }
                }
                else if (writeResult.Id == 456) // Our PC bits write operation
                {
                    Console.WriteLine($"  -> PC bits write for {writeResult.Epc.ToHexString()} complete. Status: {writeResult.Result}");
                }
            }
        }
    }
